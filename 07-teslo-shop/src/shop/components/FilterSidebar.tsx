...
export const FilterSidebar = () => {
  ...
  const handleSizeChanged = (size: string) => {
    // Si ya esta, entonces lo quitamos, si no; lo agregamos
    const newSizes = currentSizes.includes(size)
      ? currentSizes.filter((s) => s !== size)
      : [...currentSizes, size];

    // Delete param if array is empty
    ...
    searchParams.set("page", "1"); // Reiniciamos paginacion
    searchParams.set("sizes", newSizes.join(",")); // Convertimos a string para la URL, y cada filtro agregado se vera con un escape de la ',' `%2C` en la URL (pa que no saque de onda como se ve la URL)
    setSearchParams(searchParams);
  };
